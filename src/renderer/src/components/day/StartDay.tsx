...
import { createDay } from '@renderer/redux/apiCall/dayApiCAll'
const StartDay = () => {
      ...
      {error ? (
        ...
        <>
          {currentDay && (
            ...
            >
              <Chip size="lg" classNames={{ content: 'font-semibold' }} variant="flat">
                Demarée Le {getFullDate(new Date(currentDay.startAt))} à {getFullTime(new Date(currentDay.startAt))}
              </Chip>
          ...
          <div className="flex justify-center">
            {currentDay ? (
              <Button color="danger" className="font-semibold" onPress={() => console.log('stop')}>
                Stopé
              </Button>
            ) : (
              <Button
